"""TypeScript + Express code generation target (MongoDB or PostgreSQL)."""
...
class TypeScriptExpressTarget(BaseTarget):
    name = "typescript-express"
    description = "TypeScript backend with Express, database-agnostic (MongoDB or PostgreSQL)"
    ...
    def render(self, spec: "AppSpec") -> dict[str, str]:
        ...
        tmpl_names = [
            "server.ts", "models.ts", "routes.ts", "package.json",
            "tsconfig.json", "Dockerfile", "docker-compose.yml",
        ]
        if adapter.is_sql:
            tmpl_names.append("database.ts")
        if spec.auth.enabled:
            tmpl_names.append("auth.ts")
